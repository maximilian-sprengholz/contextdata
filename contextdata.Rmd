...
```{r}
key <- "AIzaSyB9VFcnDHB3XbAabqSVG3cA24iGx7dpGCo"
set_key(key = key)
```
...
Tägliche Niederschlagsdaten (1x1 km, 100 Tage in 2024 bis 09.04.).
```{r}
precip_r <- rast("data/pr_hyras_1_2024_v5-0_de.nc")
print(precip_r)
```
Nur weil es so schön ist:
```{r}
plot(precip_r[[100]], main = paste("Niederschlagssumme (mm)", time(precip_r[[100]])))
```
